...
namespace BusinessLayer
{
    public class BANGLUONG 
    {
        ...
        public void TinhLuongNhanVien(int makycong)
        {
            ...
            var lstNV = db.tb_NHANVIEN.Where(x => x.DATHOIVIEC == null).ToList();//(1)nv chưa thôi việc
            foreach(var item in lstNV)
            {
                var hd = db.tb_HOPDONG.FirstOrDefault(x => x.MANV == item.MANV && x.DELETED_BY == null);//(2)hd còn hiệu lực
                if(hd!=null)
                {
                    ...
                    luongchunhat = Convert.ToDouble(kcct.CONGCHUNHAT * luong1ngaycong * 1.5);           //
                    ...
                    bl.SONGAYTHUCLAM = int.Parse(kcct.TONGNGAYCONG.ToString());
